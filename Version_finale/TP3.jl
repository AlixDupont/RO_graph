...
function PageRank(fichier, alpha, t)
    ...
    ### lignes commentées pour le plot de la convergence

    return P
end

"""
[par Internet] renvoie les indices et valeurs de k premières valeurs d'un tableau
"""
...
"""
Affichage des plus hauts et bas PageRanks
"""
function fin_exercice_1(fichier, info_fichier)
    ...
    print(trucs_m) # normal que les valeurs aient pris un signe moins, cf ligne 187
    list_pages_M = []
    list_pages_m = []
...
"""
PageRank personnalisé, modifié pour les tests du reste de la question
l'autre option est de définir perso_PageRank(fichier, indices, valeurs)
et de définir P0 avec les valeurs aux indices
"""
function perso_PageRank(fichier, alpha, t)
    ...
    # version avec un seul indice, au-dessus : généralisé
    # P0[indices] = 1
    ...
    for time in 1:t
        ...
        P += P0 * (1 - sum(P)) # on vectorise la ligne de calcul 
    end
    return P, sum(ifelse.(real_nodes .== 0, 0, 1))
end

function perso_PageRank2(fichier, alpha, t)
    ...
    P = real_nodes
    ## même chose jusqu'ici
    ## construction du P0 particulier
    P0 = zeros(length(P))
    LC = length(nodes_chess_cleaned)
    LB = length(nodes_boxing_cleaned)

    for ic in 1:LC
        P0[nodes_chess_cleaned[ic]] += 1
    end
    for ib in 1:LB
        P0[nodes_boxing_cleaned[ib]] += 1
    end
    P0 /= (LB + LC)
...
"""
préparer les courbes pour l'exercice 2, traité ensuite en python --> plotting.py
"""
...
"""
prépare la courbe pour l'exercice 3
"""
function courbe_ex3(fichier, alpha, t)
    ...
    # on l'écrit dans un fichier pour python
...
"""
tentatives pour la fin de l'exercice 3, les modifications du vecteur personnalisé sont faites directement dans la fonction 
de PageRank personnalisé
"""
function fin_exercice_3(fichier, info_fichier, alpha, t)

    PageRanks, info = perso_PageRank(fichier, alpha, t)

    idx = collect(1:length(PageRanks))

    Nombre = 500
    trucs_M = maxk!(idx, PageRanks, Nombre, initialized = true)
    ...
    for ln in eachline(info_fichier)
        ...
        # inutile de continuer, on a eu les 5 noms
    end
    # return list_pages_M
end